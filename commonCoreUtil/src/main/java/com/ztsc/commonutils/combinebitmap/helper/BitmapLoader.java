...
import com.ztsc.commonutils.combinebitmap.cache.DiskLruCacheHelper;
import com.ztsc.commonutils.combinebitmap.cache.LruCacheHelper;
...
public class BitmapLoader {
    ...
    public void asyncLoad(final int index, final String url, final int iconDefault, final int reqWidth, final int reqHeight, final Handler handler) {

        Runnable task = new Runnable() {
            @Override
            public void run() {
                Bitmap bitmap = loadBitmap(url, iconDefault, reqWidth, reqHeight);
                if (bitmap != null) {
    ...
    private Bitmap loadBitmap(String url, int iconDefault, int reqWidth, int reqHeight) {
        ...
        try {
            if (TextUtils.isEmpty(url) || !(url.startsWith("http:") || url.startsWith("https:"))) {
                Log.e(TAG, "load from default resouse:" + url);
                bitmap = BitmapFactory.decodeResource(CommonUtil.getInstance().getApplicationContext().getResources()
                        , iconDefault, null);//将资源文件转化为bitmap
                return bitmap;
            }
            // 尝试从磁盘缓存中读取
